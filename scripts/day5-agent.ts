...
async function main() {
  ...
  try {
    const result = await runDay5Task({ taskId, task }, { planner, plannerMode: useModel ? 'openai_agent' : 'local_demo', config: loadDay4Config(), ledger, origin: process.env.DAY4_API_ORIGIN || 'http://127.0.0.1:3000' });
    console.log(JSON.stringify({ plannerMode: useModel ? 'openai_agent' : 'local_demo', ...result }, null, 2));
    if (['REJECTED', 'NEEDS_CONFIRMATION', 'PAYMENT_UNKNOWN', 'PAYING', 'APPROVED'].includes(result.status)) process.exitCode = 1;

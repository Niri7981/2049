...
import { loadPaymentConfig, PAYMENT_AMOUNT } from "../src/modules/payment/payment-config";
...
async function main() {
  ...
  const config = loadPaymentConfig({ SOLANA_CLUSTER: "localnet", SOLANA_LOCALNET_RPC_URL: rpcUrl,
  ...
  try {
    ...
    const requirement = selectPaymentQuote(unpaid.headers.get("PAYMENT-REQUIRED")!, config, preflight.facilitator.feePayer);
    ...
    const cliArgs = ["--import", resolve("node_modules/tsx/dist/loader.mjs"), resolve("scripts/pay.ts")];

...
async function main() {
  ...
  const before = await runDay4Preflight(config);
  ...
  try {
    for (let i = 1; i <= 5; i++) {
      ...
      const options = { config, origin, ledger,
        ...
        preflight: async (c: typeof config) => { stage = `run-${i}:preflight`; return runDay4Preflight(c); },
      ...
      const result = await runDay5Task({ taskId, task }, options);
      ...
      try {
        ...
        const cached = await runDay5Task({ taskId, task }, { ...options, ledger: reopened, planner: { plan: forbidden }, preflight: forbidden, pay: forbidden, answer: forbidden });
    ...
    try {
      const resource = createStaticResourceRegistry({ endpoint: 'https://day6.local.invalid/api/paid/market-snapshot', asset_id: config.mint, network: config.network, allowed_pay_to: config.merchant })[0];
    ...
    const after = await runDay4Preflight(config);

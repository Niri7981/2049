...
import { loadDay4Config, DEVNET_GENESIS, TOKEN_PROGRAM } from '../src/modules/payment/day4-config';
import { getStandardTokenAccount } from '../src/modules/payment/day4-preflight';
import { day4Rpc, confirmDay4Transaction } from '../src/modules/payment/day4-payment';
...
export async function setupDay4Accounts(directory = '.data') {
  const config = loadDay4Config();
  if (config.cluster !== 'devnet' || await day4Rpc(config, 'getGenesisHash') !== DEVNET_GENESIS) throw new Error('Account setup requires verified Devnet');
  ...
  try {
    ...
    if (saved) {
      ...
      await confirmDay4Transaction(config, saved.transaction);
    ...
    const result = await day4Rpc<{ value: Array<Account | null> }>(config, 'getMultipleAccounts', [[config.mint, ...atas], { encoding: 'jsonParsed', commitment: 'confirmed' }]);
    ...
    const balance = await day4Rpc<{ value: number }>(config, 'getBalance', [config.buyer, { commitment: 'confirmed' }]);
    const rent = await day4Rpc<number>(config, 'getMinimumBalanceForRentExemption', [165]);
    ...
    const latest = await day4Rpc<{ value: { blockhash: string; lastValidBlockHeight: number } }>(config, 'getLatestBlockhash', [{ commitment: 'confirmed' }]);
    ...
    const simulation = await day4Rpc<{ value: { err: unknown } }>(config, 'simulateTransaction', [getBase64EncodedWireTransaction(compileTransaction(message)), { encoding: 'base64', sigVerify: false, commitment: 'confirmed' }]);
    ...
    const submitted = await day4Rpc<string>(config, 'sendTransaction', [getBase64EncodedWireTransaction(transaction), { encoding: 'base64', skipPreflight: false, maxRetries: 0, preflightCommitment: 'confirmed' }]);
    if (submitted !== signature) throw new Error('Unexpected submitted transaction');
    await confirmDay4Transaction(config, signature);
    console.log(JSON.stringify({ accountsReady: true, created: missing.length, atas, transaction: signature }, null, 2));
  } finally { lock.close(); }
}
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) setupDay4Accounts().catch(error => {

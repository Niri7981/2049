...
import { createDemoKeychain, readDemoKeychain } from "../src/modules/payment/day4-keychain";
import { loadDay4Buyer } from "../src/modules/payment/day4-wallet";
import { DEVNET_USDC_MINT } from "../src/modules/payment/day4-config";
...
async function main() {
  ...
  try {
    ...
    await loadDay4Buyer(wallet.address, { DEMO_BUYER_KEYCHAIN_SERVICE: wallet.keychainService });
    ...
    console.log(JSON.stringify({ cluster: "devnet", buyer: wallet.address, signer: "macOS Keychain", reused: Boolean(saved),
      ...
      next: "Transfer test funds to this buyer, then run npm run day4:accounts and npm run day4:preflight" }, null, 2));
  } finally { lock.close(); }
}
main().catch(() => {

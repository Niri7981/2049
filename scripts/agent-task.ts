...
async function main() {
  ...
  try {
    ...
    if (['DENIED', 'REQUIRES_APPROVAL', 'REJECTED', 'NEEDS_CONFIRMATION', 'PAYMENT_UNKNOWN', 'PAYING', 'APPROVED', 'EXPIRED', 'FAILED'].includes(result.status) || ('answerStatus' in result && result.answerStatus === 'FAILED')) process.exitCode = 1;

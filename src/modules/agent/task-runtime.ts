...
export async function runTask(input: { taskId: string; task: string }, options: {
  ...
  if (config.cluster !== 'devnet' || config.network !== DEVNET_NETWORK || config.mint !== DEVNET_USDC_MINT) throw new Error('Purchase policy only supports Circle USDC on Devnet');
  ...
  const quote = selectPaymentQuote(header, config, preflight.facilitator.feePayer);

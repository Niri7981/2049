...
import { DEVNET_NETWORK, PAYMENT_AMOUNT, type Day4Config } from "./day4-config";

export const MARKET_RESOURCE = "/api/paid/market-snapshot?asset=SOL";

export function selectDay4Quote(encoded: string, config: Day4Config, feePayer: string) {
  ...
  if (requirement.scheme !== "exact" || requirement.network !== config.network ||
    ...
    throw new Error("Quote does not match the fixed Day 4 payment");
  }
  return requirement as PaymentRequirements;
}

export async function day4Rpc<T>(config: Pick<Day4Config, "rpcUrl">, method: string, params: unknown[] = []): Promise<T> {
...
export async function prepareDay4Payment(
  config: Day4Config, signer: TransactionSigner, requirement: PaymentRequirements,
  onSimulation: () => void = () => {},
  ...
  if (requirement.scheme !== "exact" || requirement.network !== config.network || requirement.asset !== config.mint ||
    ...
    throw new Error("Signer rejected a payment outside the fixed Day 4 configuration");
  }
  ...
  const checkedSigner: TransactionSigner = {
    ...
    async signTransactions(transactions, signingConfig) {
      for (const transaction of transactions) {
        const result = await day4Rpc<{ value: { err: unknown } }>(config, "simulateTransaction", [
...
export async function confirmDay4Transaction(config: Day4Config, transaction: string) {
  for (let attempt = 0; attempt < 15; attempt++) {
    const result = await day4Rpc<{ value: Array<null | { err: unknown; confirmationStatus: string }> }>(

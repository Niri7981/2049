...
```bash
npm run day4:server
npm run day6:acceptance
```

验收脚本会执行当天固定 ID 的五个任务，每个任务付款 0.01 测试 USDC，完成后通过另一 SQLite 连接验证原任务重跑不调用模型或付款。重复运行脚本会复用当天已有任务，输出 `fresh` 说明是否为新任务。需要独立新批次时可运行 `npm run day6:acceptance -- BATCH`；每个新批次最多再购买五次，切勿用新批次绕过未知付款。

...
| `src/modules/agent/task-runtime.ts` | 编排发现、报价、购买创建、Policy、付款、缓存结果及简短数据摘要。 |
| `scripts/agent-task.ts` | 开发者命令入口，选择真实模型或明确标注的本地规划模式。 |
| `scripts/run.mjs` | 新增 Day 5 入口，复用既有代理设置。 |
| `src/modules/agent/openai-capability-planner.ts` | 限制模型轮次和超时，模型不取得付款权限。 |
| `tests/unit/policy-ledger.test.ts` | 规则边界、预占预算、唯一领取、持久化与崩溃保护。 |
| `tests/unit/approved-payment.test.ts` | 无批准/绑定改变不读签名器，超时不重签，错误回执不记成功。 |
| `tests/integration/task-runtime.test.ts` | 发现到付款的受控依赖验收、并发一次购买、重跑缓存、不相关任务零付款。 |
...
- Day 6 已加入原交易只读对账和付款后数据恢复；重新运行原任务触发。仍无法确认的交易继续冻结，禁止删除账本重付。详见 `task-acceptance-runbook.md`。
- `.data/day5-ledger.sqlite` 含内部购买记录和签名载荷，不上传 Git，也不暴露给模型。开发工具和同一登录用户下的其他进程仍属于可信本机环境。

## 自动购买 收尾补充（2026-09-09）

- 已补齐过期审批预算释放、模型分析与回答缓存；原任务重跑可恢复失败的分析，不再发现资源或付款。
- `npm run model:check` 独立检查真实模型对 SOL、BTC、解释问题和转账指令的分类，不访问钱包、不购买数据；不符合预期时返回非零。
...
- 本次是模型接入及已有付款数据的分析验收，不是一次新任务从模型发现到 Devnet 付款的连续 E2E；连续完整 E2E 的独立记录见 `task-acceptance-runbook.md`。

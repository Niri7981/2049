...
开发修改使用 `npm run day4:server`；生产演示使用 `day7:demo`，避免开发工具出现在演示页面。

## 文件链路与 review 重点

```text
src/app/page.tsx → src/app/day7-demo.tsx
  → POST /api/demo/tasks
  → local-request.ts（本机、同源、请求体上限）
  → demo-store.ts（原任务锁定、启动令牌、事件持久化）
  → scripts/day7-worker.ts（固定后台入口）
  → day5-agent-runtime.ts（模型判断、资源、报价、消费规则）
...
| 文件 | 重点关键字 | 主要职责 |
| --- | --- | --- |
| `src/app/day7-demo.tsx` | `Day7Demo`、`run`、`newTask` | 输入、每 1.5 秒读取记录、刷新恢复、结果复用；没有签名代码 |
...
| `scripts/day7-worker.ts` | `trace`、`display` | 复用原 runtime；只提取回答、交易 ID、金额和快照摘要 |
| `src/modules/agent/day5-agent-runtime.ts` | `trace`、`result` | 在真实阶段写记录，不用模拟进度；单独标注未知/失败付款 |
...
| `tests/unit/day7-demo.test.ts` | `local wallet boundary`、`durable execution` | 跨站、重复提交、重启、旧令牌、脱敏及 Console/网页事件一致性 |

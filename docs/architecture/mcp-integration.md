...
没有开放付款、修改额度、管理连接、导出密钥或通用签名工具。原对话确认尚未实现，所以 M4 保持部分完成；不能把工具成功调用当成用户同意付款。
...
连接默认关闭。App 每次启用时产生新令牌，撤销立即使旧令牌失效；App 重启后保持关闭，需要重新启用。MCP 进程首次调用时缓存当前令牌，撤销后不会自行读取新令牌来恢复权限；重新授权后需要重启 MCP 会话。

App 显示“最近收到请求”，不把启用开关当作宿主已连接的证明，也不根据客户端自报名称断言调用者就是 Codex。此机制不防御同一 macOS 用户下能读取本机文件的恶意进程。

## 验证与限制

自动化覆盖官方 SDK 握手、工具发现、并发只读调用、不存在的付款工具、错误脱敏、Agent/管理权限隔离、跨站拒绝、撤销/重启后的凭据失效，以及报价条件被修改时拒绝。

本轮验证：全套 228 项测试通过后，补充的真实 stdio 子进程集成测试也通过（合计 229 项）。类型检查、lint 和 Next.js 生产构建通过。Electron 实际启动并显示 Agent 连接入口；本机 Agent 和管理接口的无凭据请求均返回 401。前端构建产物未检出签名器、钥匙串读取函数或连接/管理凭据标识。

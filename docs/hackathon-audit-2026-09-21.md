...
> 后续实现更新：本报告以提交 `0267a7d` 为审计基线。P0-1 的有限 `SpendGrant` 已完成；随后接通 `request_purchase → 真实 402 → SpendGrant → Policy → APPROVED / DENIED`，但明确停在 `paymentStatus: NOT_STARTED`。报告中“当前缺失”的描述保留为基线证据。实际 Codex 宿主验收、签名付款和新的链上交易仍未完成。

...
import { DEVNET_GENESIS, DEVNET_NETWORK, DEVNET_USDC_MINT, loadDay4Config, TOKEN_PROGRAM } from "../../src/modules/payment/day4-config";
import { getStandardTokenAccount, runDay4Preflight } from "../../src/modules/payment/day4-preflight";
...
describe("Day 4 network and wallet configuration", () => {
  it("defaults to Devnet with official USDC and a public facilitator", () => {
    expect(loadDay4Config(env)).toMatchObject({ cluster: "devnet", network: DEVNET_NETWORK,
  ...
  it.each(["mainnet", "mainnet-beta", "testnet", "unknown"])("rejects cluster %s", (cluster) => {
    expect(() => loadDay4Config({ ...env, SOLANA_CLUSTER: cluster })).toThrow("mainnet is disabled");
  });

  it("rejects invalid, missing, and identical wallet addresses", () => {
    expect(() => loadDay4Config({ ...env, DEMO_BUYER_PUBLIC_KEY: "not-a-wallet" })).toThrow("valid Solana public key");
    expect(() => loadDay4Config({ DEMO_BUYER_PUBLIC_KEY: BUYER })).toThrow("DEMO_MERCHANT_PUBLIC_KEY is required");
    expect(() => loadDay4Config({ ...env, DEMO_MERCHANT_PUBLIC_KEY: BUYER })).toThrow("different wallets");
  });
  ...
  it.each(["http://localhost.evil.example:8899", "http://127.0.0.1.evil.example", "http://localhost@evil.example"])(
    "rejects a hostname that merely starts with loopback: %s", (url) => {
      expect(() => loadDay4Config({ ...env, SOLANA_CLUSTER: "localnet", SOLANA_LOCALNET_RPC_URL: url,
  ...
  it("requires a local facilitator explicitly and accepts IPv6 loopback", () => {
    ...
    expect(() => loadDay4Config(localEnv)).toThrow("X402_FACILITATOR_URL is required");
    expect(loadDay4Config({ ...localEnv, X402_FACILITATOR_URL: "http://127.0.0.1:4022" }).rpcUrl).toBe("http://[::1]:8899");
  });

  it("permits an HTTPS Devnet provider without devnet in its URL, leaving chain verification to RPC", () => {
    expect(loadDay4Config({ ...env, SOLANA_DEVNET_RPC_URL: "https://rpc.example/secret" }).rpcUrl).toBe("https://rpc.example/secret");
    expect(() => loadDay4Config({ ...env, SOLANA_DEVNET_RPC_URL: "http://rpc.example" })).toThrow("HTTPS");
  });
});

describe("Day 4 read-only preflight", () => {
  it("accepts exactly 0.01 USDC with a sponsored fee payer and checks only standard ATAs", async () => {
    const test = fixture();
    const config = loadDay4Config({ ...env, SOLANA_DEVNET_RPC_URL: "https://rpc.example/private-key?token=secret" });
    const result = await runDay4Preflight(config, { fetch: test.fetcher });
  ...
  it("rejects a different chain even when the RPC URL says devnet", async () => {
    ...
    await expect(runDay4Preflight(loadDay4Config(env), { fetch: test.fetcher })).rejects.toThrow("does not match Solana Devnet");
  ...
  it("verifies a local network identifier against the running validator's genesis", async () => {
    ...
    const config = loadDay4Config({ ...env, SOLANA_CLUSTER: "localnet", LOCAL_USDC_MINT: DEVNET_USDC_MINT,
      SOLANA_LOCALNET_NETWORK: "solana:22222222222222222222222222222222", X402_FACILITATOR_URL: "http://127.0.0.1:4022" });
    await expect(runDay4Preflight(config, { fetch: test.fetcher })).rejects.toThrow("configured local test chain");
    expect(test.fetcher).toHaveBeenCalledTimes(1);
  });
  ...
  it.each(["version", "scheme", "network"])("rejects a facilitator with the wrong %s", async (field) => {
    ...
    await expect(runDay4Preflight(loadDay4Config(env), { fetch: test.fetcher })).rejects.toThrow("does not support x402 v2 exact");
  });

  it("requires the facilitator to advertise a valid sponsor", async () => {
    const test = fixture();
    test.state.kinds[0].extra.feePayer = "secret-invalid-address";
    await expect(runDay4Preflight(loadDay4Config(env), { fetch: test.fetcher })).rejects.toThrow("valid feePayer");
  });
  ...
  it("fails when the facilitator is unavailable even if the RPC is healthy", async () => {
    ...
    await expect(runDay4Preflight(loadDay4Config(env), { fetch: test.fetcher })).rejects.toThrow("Facilitator supported request failed or timed out");
  });
  ...
  it.each(["program", "decimals", "uninitialized"])("rejects invalid mint %s", async (field) => {
    ...
    await expect(runDay4Preflight(loadDay4Config(env), { fetch: test.fetcher })).rejects.toThrow("classic SPL token with 6 decimals");
  });

  it("rejects a buyer with 9,999 base units in the standard ATA", async () => {
    const test = fixture();
    test.buyer.data.parsed.info.tokenAmount.amount = "9999";
    await expect(runDay4Preflight(loadDay4Config(env), { fetch: test.fetcher })).rejects.toThrow("less than 0.01 USDC");
  });
  ...
  it.each(["missing", "frozen", "wrong owner", "wrong mint"])("rejects a merchant ATA that is %s", async (condition) => {
    ...
    await expect(runDay4Preflight(loadDay4Config(env), { fetch: test.fetcher })).rejects.toThrow("Merchant standard associated token account");
  });

  it.each([0, 1, 10_000])("rejects an insufficiently funded fee payer (%i lamports)", async balance => {
    const test = fixture();
    test.state.feeLamports = balance;
    await expect(runDay4Preflight(loadDay4Config(env), { fetch: test.fetcher })).rejects.toThrow("fee payer must have SOL");
  });
  ...
  it("does not expose secrets from network failures or provider errors", async () => {
    ...
    await expect(runDay4Preflight(loadDay4Config(env), { fetch: rejectingFetch })).rejects.toThrow("RPC getGenesisHash request failed or timed out");
    const errorFetch = vi.fn<typeof fetch>().mockResolvedValue(Response.json({ error: { message: "provider-secret" } }));
    await expect(runDay4Preflight(loadDay4Config(env), { fetch: errorFetch })).rejects.toThrow("RPC getGenesisHash returned an error");
  });
});

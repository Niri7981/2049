...
import { DEVNET_NETWORK, DEVNET_USDC_MINT, PAYMENT_AMOUNT, type Day4Config } from "../../src/modules/payment/day4-config";
import { confirmDay4Transaction, day4Rpc, MARKET_RESOURCE, prepareDay4Payment, selectDay4Quote } from "../../src/modules/payment/day4-payment";
...
describe("Day 4 fixed payment quote", () => {
  it("accepts only the configured quote for the market resource", () => {
    expect(selectDay4Quote(encodeQuote(), config, feePayer)).toEqual(requirement());
  });
  ...
  it.each([
    ...
    expect(() => selectDay4Quote(encodeQuote(payment), config, feePayer)).toThrow();
  });
  ...
  it.each([
    ...
    expect(() => selectDay4Quote(encodeQuote({ ...requirement(), extra }), config, feePayer)).toThrow();
  });

  it("rejects a changed resource and ambiguous payment options", () => {
    expect(() => selectDay4Quote(encodeQuote(requirement(), "/api/other"), config, feePayer)).toThrow();
    expect(() => selectDay4Quote(encodeQuote(requirement(), MARKET_RESOURCE, 2), config, feePayer)).toThrow();
  });

  it("rejects malformed and oversized quote headers", () => {
    expect(() => selectDay4Quote("not-json", config, feePayer)).toThrow();
    expect(() => selectDay4Quote("a".repeat(16_385), config, feePayer)).toThrow("too large");
  });
});

describe("Day 4 simulation before signing", () => {
  ...
  it("does not call the underlying signer after simulation failure", async () => {
    ...
    await expect(prepareDay4Payment(config, signer, requirement(), onSimulation)).rejects.toThrow("nothing was signed or submitted");
  ...
  it("does not sign if simulation RPC fails", async () => {
    ...
    const { signer, signTransactions } = setupSigning();
    await expect(prepareDay4Payment(config, signer, requirement())).rejects.toThrow("RPC simulateTransaction failed");
    expect(signTransactions).not.toHaveBeenCalled();
  });
  ...
  it("does not sign when the simulation response omits its error status", async () => {
    mockRpc({ value: {} });
    const { signer, signTransactions } = setupSigning();
    await expect(prepareDay4Payment(config, signer, requirement())).rejects.toThrow("nothing was signed or submitted");
    expect(signTransactions).not.toHaveBeenCalled();
  });
  ...
  it("signs after a successful simulation and discards quote RPC hints", async () => {
    ...
    const result = await prepareDay4Payment(config, signer, payment, onSimulation);
  ...
  it("rejects a buyer address mismatch before invoking the SDK", async () => {
    const { signer, signTransactions } = setupSigning();
    await expect(prepareDay4Payment({ ...config, buyer: config.merchant }, signer, requirement())).rejects.toThrow("matching the configured buyer");
  ...
  it("rechecks the fixed amount inside the signing boundary", async () => {
    const { signer, signTransactions } = setupSigning();
    await expect(prepareDay4Payment(config, signer, { ...requirement(), amount: "1000000" })).rejects.toThrow("outside the fixed");
    expect(sdk.constructor).not.toHaveBeenCalled();
    expect(signTransactions).not.toHaveBeenCalled();
  });
});

describe("Day 4 transaction confirmation", () => {
  it.each(["confirmed", "finalized"])("accepts %s only with no chain error", async confirmationStatus => {
    ...
    await expect(confirmDay4Transaction(config, "test-signature")).resolves.toBe(confirmationStatus);
  });
  ...
  it("rejects a chain error even if the transaction is confirmed", async () => {
    ...
    await expect(confirmDay4Transaction(config, "test-signature")).rejects.toThrow("failed on chain");
  });

  it("rejects confirmed responses that omit the transaction error field", async () => {
    mockRpc({ value: [{ confirmationStatus: "confirmed" }] });
    await expect(confirmDay4Transaction(config, "test-signature")).rejects.toThrow("invalid status");
  });
  ...
  it.each([null, { err: null, confirmationStatus: "processed" }])("keeps %j unknown instead of reporting success", async status => {
    ...
    const confirmation = confirmDay4Transaction(config, "test-signature").catch(error => error);
  ...
  it("surfaces transport and RPC failures without leaking provider error details", async () => {
    ...
    await expect(day4Rpc(config, "getSignatureStatuses")).rejects.toThrow("HTTP 503");
    vi.stubGlobal("fetch", vi.fn().mockResolvedValue(new Response(JSON.stringify({ error: { message: "secret-provider-diagnostic" } }))));
    await expect(confirmDay4Transaction(config, "test-signature")).rejects.toThrow(/^RPC getSignatureStatuses failed$/);
  });
});

...
function fixture() {
  ...
  ledger.exec(`
    CREATE TABLE purchases (id TEXT PRIMARY KEY, task_id TEXT UNIQUE, approval_id TEXT, purchase TEXT, quote TEXT, decision TEXT,
      status TEXT, amount INTEGER, confirmed_day TEXT, transaction_id TEXT, data TEXT, payload TEXT);
  ...
  ledger.prepare('INSERT INTO purchases VALUES (?,?,?,?,?,?,?,?,?,?,?,?)').run(
    ...
    JSON.stringify({ amount: '200000', network: config.network, asset: config.mint, payTo: config.merchant, extra: { memo: 'day4:AAAAAAAAAAAAAAAAAAAAAA' } }),
    JSON.stringify({ decision: 'APPROVED', reason: 'AUTHORITY_BUDGET_AND_GRANT_PASSED' }),
    'PAID', 200000, '2026-09-22', signature, basicData, 'raw-payment-payload-must-never-appear',
  );
  ledger.prepare('INSERT INTO purchases VALUES (?,?,?,?,?,?,?,?,?,?,?,?)').run(
    ...
    'DENIED', 20000000, null, null, null, null,
  );
  ledger.exec(`
...
it('reports successful purchase evidence without exposing signer or authorization material', async () => {
  ...
  const evidence = await collectE2eEvidence('basic-request', {
    dataDirectory: value.directory, settlementDatabase: value.settlementPath, config, liveDevnetEnabled: true, fetcher: value.fetcher,
  });

...
用户必须先自行填写每日共用额度。测试购买入口默认只走报价约束、后端策略、状态转换和持久账本，不签名、不提交交易，并在界面明确标记。真实 Devnet 模式虽然已接到原购买后端，但默认关闭，未经具体付款授权不要启用。实现选择和生命周期边界见 [macOS App 壳方案](docs/architecture/macos-app-shell.md)。
